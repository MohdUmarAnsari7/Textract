...
def main():
    ...
    if email and password:
        ...
        if user:
            ...
            return "User not found"  # Indicate that the email does not exist
    else:
        return render_template("main.html")
...
def registerSuccess():
    ...
    if username and password and email:
        ...
        if collection.find_one({"username": username}):
            return render_template('registration.html', message="Username already exists!", status=400)
        if collection.find_one({"email": email}):
            return render_template('registration.html', message="Email already registered!", status=400)
